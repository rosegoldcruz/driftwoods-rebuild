...
const applyParticleEffect = (element: HTMLElement, options?: CoolParticleOptions): (() => void) => {
  ...
  const limit = options?.particleCount ?? 40
  ...
  function generateParticle() {
    ...
    const speedHorz = options?.speedHorz || Math.random() * 9
    const speedUp = options?.speedUp || Math.random() * 20
    const spinVal = Math.random() * 360
    const spinSpeed = Math.random() * 30 * (Math.random() <= 0.5 ? -1 : 1)
    ...
    if (particleType === 'circle') {
      ...
      circle.setAttributeNS(null, 'fill', `hsl(${18 + Math.random() * 20}, 85%, 56%)`)
...
export function CoolMode({ children, options, className = 'inline-block' }: CoolModeProps) {
